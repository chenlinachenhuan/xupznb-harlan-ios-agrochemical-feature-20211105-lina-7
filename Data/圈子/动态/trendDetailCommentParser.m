...
@implementation trendDetailCommentParser
    ...
    [self PostRequestDataWithControlParam:dic url:Request_trendcommentList BaseFinishBlock:^(NSDictionary * _Nonnull dataDic) {
        if ([dataDic[@"code"] intValue]==200) {
            ...
            for (NSDictionary * dic in dataDic[@"data"][@"list"]) {
                trendCommentModel * model = [[trendCommentModel alloc] initWithData:dic];
                [arr addObject:model];
            }
            [self.myDelegate successTrendCommentListArr:arr withTotal:dataDic[@"data"][@"total"]];
        }else{
