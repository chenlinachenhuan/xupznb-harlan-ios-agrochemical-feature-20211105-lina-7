...
@interface WJPlayerView()<UIGestureRecognizerDelegate>
...
-(void)showCell:(UIView*)view{
//    if (self.movieURL.path.length == 0) {
//        [WJMovieHUD showWithMessage:@"视频播放地址不存在"];
//        return;
//    }
    self.frame = view.bounds;
    NSLog(@"%@",self.transitionView);
//    self.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0];
    [self addSubview:self.transitionView];
    [self addSubview:self.playerView];
...
- (void)loadData {
   self.task = [[WJMovieDownLoadManager shareManager] downloadMovieWithURL:_movieURL progressBlock:^(CGFloat progress) {
        self.progressView.progress = progress;
    } success:^(NSURL *URL) {
        [self.progressView removeFromSuperview];
        self.playerURL = URL;
        self.playerView.player = [[AVPlayer alloc] initWithURL:URL];
        NSLog(@"%@====%@",self.playerView,URL);
