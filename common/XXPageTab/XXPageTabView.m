...
//            tabItem.backgroundColor = SelectColor;
...
- (void)layoutIndicatorView {
    ...
    NSLog(@"%f",selecedTabItem.frame.origin.x);
    NSLog(@"%f==%f==%f",selecedTabItem.center.x,indicatorWidth/2.0,_tabView.contentOffset.x);
    self.indicatorView.frame = CGRectMake(_tabFrame.origin.x+selecedTabItem.center.x-indicatorWidth/2.0-_tabView.contentOffset.x, _tabSize.height-_indicatorHeight, indicatorWidth, _indicatorHeight);
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if(scrollView == self.tabView) {
        ...
        if(self.indicatorView.superview) {
            XXPageTabItemLable *selecedTabItem = _tabItems[_selectedTabIndex];
            self.indicatorView.frame = CGRectMake(_tabFrame.origin.x+selecedTabItem.center.x-WIDTH(self.indicatorView)/2.0-scrollView.contentOffset.x, ORIGIN_Y(self.indicatorView), WIDTH(self.indicatorView), HEIGHT(self.indicatorView));
        }
...
- (void)changeIndicatorFrame {
    ...
    self.indicatorView.frame = CGRectMake(_tabFrame.origin.x+nowIndicatorCenterX-nowIndicatorWidth/2.0-_tabView.contentOffset.x, ORIGIN_Y(self.indicatorView), nowIndicatorWidth, HEIGHT(self.indicatorView));
}
...
- (void)changeIndicatorFrameByStretch {
    ...
    if(relativeLocation <= 0.5) {
        ...
        nowFrame.origin.x = (_tabFrame.origin.x+leftTabItem.center.x-self.tabView.contentOffset.x)-_indicatorWidth/2.0;
    } else {
        nowFrame.size.width = _indicatorWidth+_tabItemWidth*((1-relativeLocation)/0.5);
        nowFrame.origin.x = (_tabFrame.origin.x+rightTabItem.center.x-self.tabView.contentOffset.x)+_indicatorWidth/2.0-nowFrame.size.width;
    }
