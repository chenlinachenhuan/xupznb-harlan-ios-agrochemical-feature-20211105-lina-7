...
@interface policyViewController ()<policyDetailParserDelegate,UIGestureRecognizerDelegate,UIScrollViewDelegate,UITextFieldDelegate,WKNavigationDelegate,policyCommentListParserDelegate,policyCreateCommentParserDelegate,policyCommentFabulousParserDelegate,policyCommentCancelFabulousParserDelegate>
...
-(void)requestNewsDetailTextMethod{
    ...
    policyDetailParser * parser = [[policyDetailParser alloc] init];
    parser.myDelegate = self;
    [parser requestPolicy:[ self.myId integerValue] ];
}
...
//    [[ChrysanthemumIndexView sharedMasterUser] show];
    policyCommentListParser * parser = [[policyCommentListParser alloc] init];
    parser.myDelegate = self;
    [parser requestPolicyComment:[ _myId integerValue] withOrderStatus:0 withPageNum:1 pageSize:10 parentId:0 pubTimeStatus:0 showStatus:1];
}
...
-(void)likeButtonMethod:(newsLikeButton*)button{
    ...
    if (button.selected==1) {
        policyCommentFabulousParser * parser = [[policyCommentFabulousParser alloc] init];
        parser.myDelegate = self;
        [parser requestWithId:model.myId];
    }else{
        policyCommentCancelFabulousParser * parser = [[policyCommentCancelFabulousParser alloc] init];
        parser.myDelegate = self;
        [parser requestWithId:model.myId];
    }
...
-(void)reportButtonMethod:(UIButton*)button{
    newsCommentFrameModel * frameM = self.commentFrameArr[button.tag];
    newsCommentListTextModel * model = frameM.model;
    policyCommentReportParser * parser = [[policyCommentReportParser alloc] init];
    parser.myDelegate = self;
    [parser requestWithId:model.myId];
}
-(void)successReport{
...
-(void)sendButtonMethod{
    [self hideKeyBoard];
    policyCreateCommentParser * parser = [[policyCreateCommentParser alloc] init];
    parser.myDelegate = self;
