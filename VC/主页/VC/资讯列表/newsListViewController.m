...
@interface newsListViewController ()<UIScrollViewDelegate,UIGestureRecognizerDelegate,newsProtocolDelegate,newsCatListParserDelegate,newsListViewDelegate>
...
//    [[ChrysanthemumIndexView sharedMasterUser] show];
    if (!self.newsP) {
        self.newsP = [[newsParser alloc] initWithViewProtocol:self];
    }
    knowledgeCatListModel*model = self.cat2NameArr[_selectIndex];
    [self.newsP requestWithCat2Id:[model.myId integerValue] withCatId:0 withKeyWord:@"" withPageNum:1 withPageSize:model.pageSize withShowStatus:1];
...
-(void)createNewsListView{
    ...
    self.ClassificationView = [[newsCollectionView alloc] initWithFrame:CGRectMake(16, TopHeight+16, ScreenWidth-32, 0.369942197*ScreenWidth*0.23066667) collectionViewLayout:flow];
