...
@interface richDetailViewController ()<richDetailParserDelegate,UIGestureRecognizerDelegate,UIScrollViewDelegate,UITextFieldDelegate,WKNavigationDelegate,richCommentListParserDelegate,richCreateCommentParserDelegate,richCommentFabulousParserDelegate,richCommentCancelFabulousParserDelegate,richCommentReportParserDelegate>
...
@implementation richDetailViewController
...
-(void)requestNewsDetailTextMethod{
    ...
    richDetailParser * parser = [[richDetailParser alloc] init];
    parser.myDelegate = self;
    [parser requestRich:[ self.myId integerValue] ];
}
...
-(void)requestNewsDetailCommentMethod{
    [[ChrysanthemumIndexView sharedMasterUser] show];
    richCommentListParser * parser = [[richCommentListParser alloc] init];
    parser.myDelegate = self;
    [parser requestRichComment:[ _myId integerValue] withOrderStatus:0 withPageNum:1 pageSize:self.pageSize parentId:0 pubTimeStatus:0 showStatus:1];
}
...
-(void)likeButtonMethod:(newsLikeButton*)button{
    ...
    if (button.selected==1) {
        richCommentFabulousParser * parser = [[richCommentFabulousParser alloc] init];
        parser.myDelegate = self;
        [parser requestWithId:model.myId];
    }else{
        richCommentCancelFabulousParser * parser = [[richCommentCancelFabulousParser alloc] init];
        parser.myDelegate = self;
        [parser requestWithId:model.myId];
    }
...
-(void)reportButtonMethod:(UIButton*)button{
    newsCommentFrameModel * frameM = self.commentFrameArr[button.tag];
    newsCommentListTextModel * model = frameM.model;
    richCommentReportParser * parser = [[richCommentReportParser alloc] init];
    parser.myDelegate = self;
    [parser requestWithId:model.myId];
}
-(void)successReport{
...
-(void)sendButtonMethod{
    [self hideKeyBoard];
    richCreateCommentParser * parser = [[richCreateCommentParser alloc] init];
    parser.myDelegate = self;
